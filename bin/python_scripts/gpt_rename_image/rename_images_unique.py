...
# Function to generate a random suffix with n digits
def generate_random_suffix(num_digits=4):
    range_start = 10**(num_digits-1)
    range_end = (10**num_digits) - 1
    return f"{random.randint(range_start, range_end)}"

# Function to rename a single image
def rename_image(image_path):
    ...
    if suggested_name:
        ...
        new_filename = f"{sanitized_name}_{random_suffix}"
    else:
